...
server.listen(port, function () {
    logger.info("DVP-QAModule.main Server %s listening at %s", server.name, server.url);
});


server.post('/DVP/API/:version/QAModule/Question',authorization({resource:"qualityassurance", action:"write"}), function(req, res, next){
        ...
        if(req.body) {
            ...
            question.save(function (err, question) {
                if (err) {
                    ...
                    var message = msg.FormatMessage(err, "Question creation failed", false, undefined);

                } else {

                    if (question) {
                        logger.info("DVP-QAModule.CreateQuestion successful");
                        var message = msg.FormatMessage(err, "Question creation successful", true, question);
                    } else {

                        logger.error("DVP-QAModule.CreateQuestion failed ");
                        var message = msg.FormatMessage(undefined, "Question creation failed", false, undefined);
                    }
                }
                res.write(message);
                res.end();
            ...
            var instance = msg.FormatMessage(undefined,"create question failed", false,undefined);
            res.write(instance);
            res.end()
        }
    }else{
...
server.post('/DVP/API/:version/QAModule/Section',authorization({resource:"qualityassurance", action:"write"}), function(req, res, next){
        ...
        if(req.body) {
            ...
            section.save(function (err, section) {
                ...
                if (err) {
                    ...
                    var message = msg.FormatMessage(err, "Section creation failed", false, undefined);

                } else {

                    if (section) {
                        logger.info("DVP-QAModule.CreateSection successful");
                        var message = msg.FormatMessage(err, "Section creation successful", true, section);


                    } else {

                        logger.error("DVP-CreateSection.CreateSection failed ");
                        var message = msg.FormatMessage(undefined, "Section creation failed", false, undefined);
            ...
            var instance = msg.FormatMessage(undefined,"Section creation failed", false,undefined);
            res.write(instance);
            res.end();
...
server.get('/DVP/API/:version/QAModule/Questions',authorization({resource:"qualityassurance", action:"read"}),function (req, res, next) {
    ...
    Question.find({
        ...
        if (err) {
            ...
            jsonString = msg.FormatMessage(err, "Get all questions failed", false, undefined);

        } else {

            if (questions) {

                jsonString = msg.FormatMessage(undefined, "Get all questions Successful", true, questions);

            } else {

                jsonString = msg.FormatMessage(undefined, "No questions Found", false, questions);
...
server.del('/DVP/API/:version/QAModule/Question/:id',authorization({resource:"qualityassurance", action:"delete"}),function (req, res, next) {
    ...
    Question.findOneAndRemove({
        ...
        if (err) {
            ...
            jsonString = msg.FormatMessage(err, "Delete question failed", false, undefined);

        } else {

            if (questions) {

                jsonString = msg.FormatMessage(undefined, "Delete question Successful", true, questions);
...
server.get('/DVP/API/:version/QAModule/Question/:id',authorization({resource:"qualityassurance", action:"read"}),function (req, res, next) {
    ...
    Question.findOne({
        ...
        if (err) {
            ...
            jsonString = msg.FormatMessage(err, "Get question failed", false, undefined);

        } else {

            if (questions) {

                jsonString = msg.FormatMessage(undefined, "Get question Successful", true, questions);
...
server.get('/DVP/API/:version/QAModule/Sections',authorization({resource:"qualityassurance", action:"read"}),function (req, res, next) {
    ...
    Section.find({
        ...
        if (err) {
            ...
            jsonString = msg.FormatMessage(err, "Get all sections failed", false, undefined);
...
server.get('/DVP/API/:version/QAModule/Section/:id',authorization({resource:"qualityassurance", action:"read"}),function (req, res, next) {
    ...
    Section.findOne({
        ...
        if (err) {
            ...
            jsonString = msg.FormatMessage(err, "Get section failed", false, undefined);

        } else {

            if (section) {

                jsonString = msg.FormatMessage(undefined, "Get section Successful", true, section);
...
server.post('/DVP/API/:version/QAModule/Paper',authorization({resource:"qualityassurance", action:"write"}), function(req, res, next){
        ...
        if(req.body) {
            ...
            paper.save(function (err, qp) {
                if (err) {
                    ...
                    var message = msg.FormatMessage(err, "Question Paper creation failed", false, undefined);

                } else {

                    if (qp) {
                        logger.info("DVP-QAModule.CreateQuestionPaper successful");
                        var message = msg.FormatMessage(err, "Question paper creation successful", true, qp);
                    } else {
            ...
            var instance = msg.FormatMessage(undefined,"create question paper failed", false,undefined);
            res.write(instance);
            res.end()
        }
    }else{
    ...
    Paper.findOne({
        ...
        if (err) {
            ...
            if (qp) {
                ...
                jsonString = msg.FormatMessage(undefined, "Get question paper Successful", true, qp);
...
server.put('/DVP/API/:version/QAModule/QuestionPaper/:id/Question',authorization({resource:"qualityassurance", action:"write"}), function(req, res, next){
    ...
    if(req&& req.user && req.user.company && req.user.tenant) {
        ...
        if(req.body) {
                    ...
                    if (qp) {
                        ...
                        question.save(function (err, question) {
                            if (err) {
                                ...
                                logger.error("DVP-QAModule.CreateQuestion failed ", err);
                                jsonString = msg.FormatMessage(err, "Question creation failed", false, undefined);
                                res.end(jsonString);
                                ...
                                if (question) {
                                    ...
                                    qp.update({
                                        ...
                                        if (err) {
                                            ...
                                            jsonString = msg.FormatMessage(undefined, "Add question to paper Successfully.", true, obj);
                                            res.end(jsonString);
                                        }
                                        res.end(jsonString);
                                    ...
                                    logger.error("DVP-QAModule.CreateQuestion failed ");
                                    jsonString = msg.FormatMessage(undefined, "Question creation failed", false, undefined);
                                    res.end(jsonString);
                                }
                            }
                        });
            ...
            jsonString = msg.FormatMessage(undefined,"add question to paper failed", false,undefined);
            res.end(jsonString)
        }
    }else{
    ...
    if(req&& req.user && req.user.company && req.user.tenant) {
        ...
        Paper.update({
            ...
            if (err) {
                ...
                jsonString = msg.FormatMessage(undefined, "Delete question from paper Successfully.", true, obj);
                res.end(jsonString);
            }
            res.end(jsonString);
        ...
        jsonString = msg.FormatMessage(undefined, "Question delete failed", false, undefined);
        res.end(jsonString);
    }
        ...
        if(req.body) {
            ...
            submission.save(function (err, sub) {
                if (err) {
                    ...
                    var message = msg.FormatMessage(err, "Question Paper submission failed", false, undefined);

                } else {

                    if (sub) {
                        logger.info("DVP-QAModule.QuestionPaperSubmission successful");
                        var message = msg.FormatMessage(err, "Question paper submission successful", true, sub);
                    } else {

                        logger.error("DVP-QAModule.QuestionPaperSubmission failed ");
                        var message = msg.FormatMessage(undefined, "Question paper submission failed", false, undefined);
                    }
                }
                res.write(message);
                res.end();
            ...
            logger.error("DVP-QAModule.QuestionPaperSubmission request.body is null");
            var instance = msg.FormatMessage(undefined,"create paper submission failed", false,undefined);
            res.write(instance);
            res.end()
        }
    }else{
...
server.put('/DVP/API/:version/QAModule/QuestionPaperSubmission/:id/QuestionAnswer',authorization({resource:"qualityassurance", action:"write"}), function(req, res, next){
    ...
    if(req&& req.user && req.user.company && req.user.tenant) {
        ...
        if(req.body) {
                    ...
                    if (sub) {
                        ...
                        answer.save(function (err, answer) {
                            if (err) {
                                ...
                                jsonString = msg.FormatMessage(err, "Answer creation failed", false, undefined);
                                res.end(jsonString);
                                ...
                                if (answer) {
                                    ...
                                    sub.update({
                                        ...
                                        if (err) {
                                            jsonString = msg.FormatMessage(err, "Fail to Find Submission", false, undefined);
                                            res.end(jsonString);
                                        }
                                        else {
                                            jsonString = msg.FormatMessage(undefined, "Add answer to submission Successfully.", true, obj);
                                            res.end(jsonString);
                                        }
                                        res.end(jsonString);
                                    ...
                                    jsonString = msg.FormatMessage(undefined, "Answer creation failed", false, undefined);
                                    res.end(jsonString);
                                }
                            }
                        });
            ...
            jsonString = msg.FormatMessage(undefined,"add answer to submission failed", false,undefined);
            res.end(jsonString)
        }
    }else{
...
server.del('/DVP/API/:version/QAModule/QuestionPaperSubmission/:id/QuestionAnswer/:qid',authorization({resource:"qualityassurance", action:"delete"}), function(req, res, next){
    ...
    if(req&& req.user && req.user.company && req.user.tenant) {
        ...
        Submission.update({
            ...
            if (err) {
                ...
                jsonString = msg.FormatMessage(undefined, "Delete answer from submission Successfully.", true, obj);
                res.end(jsonString);
            }
            res.end(jsonString);
        ...
        jsonString = msg.FormatMessage(undefined, "Answer delete failed", false, undefined);
        res.end(jsonString);
    }
...
server.put('/DVP/API/:version/QAModule/QuestionPaperSubmission/:id/Complete',authorization({resource:"qualityassurance", action:"write"}), function(req, res, next){
    ...
    if(req&& req.user && req.user.company && req.user.tenant) {
        ...
        Submission.update({
            ...
            if (err) {
                ...
                jsonString = msg.FormatMessage(undefined, "Update submission Successfully.", true, obj);
                res.end(jsonString);
            }
            res.end(jsonString);
        ...
        jsonString = msg.FormatMessage(undefined, "Update Submission failed", false, undefined);
        res.end(jsonString);
    }
    ...
    Submission.findOne({
        ...
        if (err) {
            ...
            if (sub) {
                ...
                jsonString = msg.FormatMessage(undefined, "No paper submission Found", false, undefined);
